...
def googleAds_api_create_new_campaign():
    if request.method == "POST":
        ...
        explicitly_shared = rq.get('explicitly_shared', 1)
        period = rq.get('period', 'DAILY')
        res = CampaignManagement.create_new_campaign(customer_id, budget, name, days_to_start, weeks_to_end, status, delivery_method, explicitly_shared, period, lifetime_budget)
       # try:
       #      campaign_id = res.get('body').get('id')
       #      db.addCampaign(campaign_id, ad_account_id, campaign_name, objective, status)
       #  except Exception as e:
